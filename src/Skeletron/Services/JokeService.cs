...
"Рабинович работает в Кремле - он сидит на Спасской башне и смотрит в даль, чтобы своевременно сигнализировать о приближении коммунизма. Его пытаются переманить американцы - пусть предупреждает о приближении экономических кризисов.\n - нет, - говорит Рабинович, - мне нужна постоянная работа.",
 ...
 "Самый длинный анекдот: программа строительства коммунизма, принятая на XXII съезде партии."
    ...
    private List<string> _itJokes = new()
    {
...
"Проблема с шутками про IPv6 состоит в том, что их трудно вспомнить.",
...
"Шутки про 10/100/1000BASE–T вряд ли услышат с расстояния больше 100."
    ...
    private List<string> _randomTalksJokes = new()
    {
...
"Повторно повторять все повторяющиеся однокоренные слова – это тавтология – лишнее излишество."
    ...
    {
        var randomIndex = _random.Next(0, _itJokes.Count);
        return _itJokes[randomIndex];
    }

    public string GetRandomTalksJoke()
    {
        var randomIndex = _random.Next(0, _randomTalksJokes.Count);
        return _randomTalksJokes[randomIndex];
    }
}